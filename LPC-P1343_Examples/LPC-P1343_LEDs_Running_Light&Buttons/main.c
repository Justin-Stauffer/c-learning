...
void Toggle_vLEDs_Port(unsigned long vLEDs_Select, unsigned long delay_time);
...
void init_devices(void){
  ...
  // USBC
  IOCON_PIO0_6 &= 0x0000003F; // Function PIO2_4; Disable pull-up/down; Disable Hysteresis
  GPIO0DATA |= 0x00000060;    // PIO0_6 in state "1" if output
  GPIO0DIR |= 0x00000060;     // GPIO data direction register: 1 -> Outpt, 0 -> Input
  // Buttons
  // BUT1
  // Init peripherial interrupt
  IOCON_PIO2_9 &= 0x0000003F; // Function PIO2_9; Disable pull-up/down; Disable Hysteresis
  IOCON_PIO2_9 |= 0x00000010; // Function PIO2_9; Pull-up resistor enabled
  GPIO2DATA |= 0x00000200;    // PIO2_9 in state "1" if output
  GPIO2DIR &= ~0x00000200;    // GPIO data direction register: 1 -> Outpt, 0 -> Input
  GPIO2IS &= ~0x00000200;     // Interrupt on pin PIO2_9 is configured as edge sensitive.
  GPIO2IEV |= 0x00000200;     // falling edges or LOW level on pin PIO2_9 trigger an interrupt.
  GPIO2IE  |= 0x00000200;     // Interrupt on pin PIO2_9 is not masked.
  GPIO2IC = (1UL<<9);         // Clear PIO2_9 interrupt flag. "1UL" means "unsigned long 1"
  // Init NVIC interrupt. NVIC is part of Cortex core!
  NVIC_IntEnable(NVIC_PIO_2);
  // BUT2
  // Init peripherial interrupt
  IOCON_PIO1_4 &= 0x0000003F; // Function PIO1_4; Disable pull-up/down; Disable Hysteresis
  IOCON_PIO1_4 |= 0x00000010; // Function PIO1_4; Pull-up resistor enabled
  GPIO1DATA |= 0x00000010;    // PIO1_4 in state "1" if output
  GPIO1DIR &= ~0x00000010;    // GPIO data direction register: 1 -> Outpt, 0 -> Input
  GPIO1IS &= ~0x00000010;     // Interrupt on pin PIO1_4 is configured as edge sensitive.
  GPIO1IEV |= 0x00000010;     // falling edges or LOW level on pin PIO1_4 trigger an interrupt.
  GPIO1IE  |= 0x00000010;     // Interrupt on pin PIO1_4 is not masked.
  GPIO1IC = (1UL<<4);         // Clear PIO1_4 interrupt flag. "1UL" means "unsigned long 1"
  // Init NVIC interrupt. NVIC is part of Cortex core!
  NVIC_IntEnable(NVIC_PIO_1);
...
void main(void){
  ...
  while(1){
    for(i=0;i<3;i++){
      LEDs_Running_Light('L', delay_time);
    }
    for(i=0;i<3;i++){
      LEDs_Running_Light('R', delay_time);
    }
    for(i=0;i<6;i++){
      Toggle_vLEDs_Port(0x000000FF,delay_time);
    }
...
/**********************************************************************************/
/*  Function name: Toggle_vLEDs_Port                                              */
/*  	Parameters                                                                */
/*          Input   :  vLEDs_Port                                                 */
/*          Output  :  No	                                                  */
/*	Action: Toggle virtual vLEDs_Port					  */
/**********************************************************************************/
void Toggle_vLEDs_Port(unsigned long vLEDs_Select, unsigned long delay_time){
  
  Write_LEDs_Port((~vLEDs_Select));
  Delay(delay_time);
  vLEDs_Select ^= vLEDs_Select;
  Write_LEDs_Port((~vLEDs_Select));
  Delay(delay_time);
...
void CT32B0_IRQHandler(void)
{
  ...
  NVIC_ClrPend(NVIC_CT32B0); // Some peripherals must clear and NVIC flag! 
}
...
/*          Input   :  No                                                         */
/*          Output  :  No	                                                  */
/*	Action: BUT1 interrupt subroutine       				  */
/**********************************************************************************/
...
{ 
  ...
  if(delay_time <= 1000000){
    ...
    delay_time = delay_time + 500000;
    if(delay_time == 8000000){
      delay_time = 7500000;
    }
  }
  
  while(!BUT1_Chk){};   // Wait BUT1 to be released
  Delay(10000);         // Wait BUT1 mechanical vibrations to be set. 
  // In other case you will recognize it like pressed several times!
  // This is not very clever, because you spend more time!
  // Better decision is Buttons scanning shown in next project:
  // "LPC-P1343_LEDs_Running_Light&Buttons_Scan"
  // clear interrupt flag
  GPIO2IC = (1UL<<9);
  dummy = GPIO2IC;
}
...
/*          Input   :  No                                                         */
/*          Output  :  No	                                                  */
/*	Action: BUT2 interrupt subroutine       				  */
/**********************************************************************************/
...
{ 
  ...
  while(!BUT2_Chk){}; // Wait BUT2 to be released
  Delay(10000);       // Wait BUT2 mechanical vibrations to be set. 
  // In other case you will recognize it like pressed several times!
  // This is not very clever, because you spend more time!
  // Better decision is Buttons scanning shown in next project:
  // "LPC-P1343_LEDs_Running_Light&Buttons_Scan"
